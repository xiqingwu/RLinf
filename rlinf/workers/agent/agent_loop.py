...
class AgentLoopWorker(Worker):
    ...
    async def vl_generate(
        self, messages: list[dict], sampling_params: Optional[dict] = None
    ):
        """Generate response for VL (vision-language) messages.
        ...
        Returns:
            Generation result from the VL model.
        """
        channel_key = uuid4().hex
        await self.generate_input_channel.put(
            {
    ...
    async def run_agentloop_rollout_group(
        ...
        await output_channel.put(task_results, async_op=True).async_wait()
